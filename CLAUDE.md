...
다섯 화면이 하나의 골격을 공유한다. `app/app.js` 가 셸을 세우고, 화면 모듈은 그 안의 자리를 채운다.
...
**설정(S1)은 화면이 아니라 서랍이다.** 단계 흐름(`SCREENS`)에 넣지 않는다. 넣으면 "몇 번째 단계" 셈이 틀어지고, 작업 중에 설정 하나 바꾸려고 흐름 밖으로 튕겨 나갔다 돌아와야 한다. 대화 자리만 덮고 오른쪽 캔버스는 그대로 둔다 — 무엇을 만들던 중이었는지 보이는 채로 설정을 만져야 한다. 여닫는 일은 `app.js` 의 `openSettings`/`closeSettings` 가 한다.
