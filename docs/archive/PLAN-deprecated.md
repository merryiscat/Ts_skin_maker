> # 이 계획은 폐기되었습니다
>
> **실제 구현은 `webapp/` 이고 이 문서와 정반대입니다.** 아래 1100여 줄은 React + Vite 프론트,
> FastAPI 백엔드, WebSocket, 서버측 OpenAI 호출, RAG 를 전제로 쓴 것입니다. 문서 전체에
> `webapp` 이라는 단어가 한 번도 나오지 않습니다.
>
> **왜 접었나.** 서버를 두면 사용자의 API 키를 남의 서버에 맡기라는 뜻이 되고, 운영비와
> 책임이 따라붙습니다. 개인이 만들어 그냥 쓰라고 내놓는 도구에는 맞지 않는 구조였습니다.
> 그래서 서버 없는 정적 페이지로 방향을 바꿨습니다 - 사용자가 자기 키를 자기 브라우저에
> 넣고, 호출은 브라우저에서 제공자로 직접 나가고, 결과는 파일로 내려받습니다.
>
> **그래도 지우지 않는 이유**는 "왜 백엔드를 안 만들었나"의 답이 여기 있기 때문입니다.
> 다음에 같은 고민을 할 때 처음부터 다시 따지지 않아도 됩니다.
>
> 지금의 구조는 `README.md` 와 `CLAUDE.md` 를 보세요.
