(function(){
"use strict";
function secretVirtualCheckboxClick(element) {
  var _a;
  const realCheckbox = (_a = element.parentElement) == null ? void 0 : _a.querySelector('input[data-title="secret-checkbox"]');
  if (realCheckbox === void 0 || realCheckbox === null) return;
  realCheckbox.checked = !realCheckbox.checked;
}
window.secretVirtualCheckboxClick = secretVirtualCheckboxClick;
function m(t, e) {
  return e * 100 / t;
}
function A(t) {
  const { from: e, to: s, positionOption: o = { type: "right", align: "center" }, margin: n = 0 } = t, i = S(s), c = S(e), r = N(i, c, o, n);
  e.style.position = "fixed", e.style.opacity = "1", e.style.visibility = "visible", e.style.top = `${r.top}px`, e.style.left = `${r.left}px`;
}
function S(t) {
  return {
...
function N(t, e, s, o) {
  const n = t.rect.width, i = t.rect.height, c = t.rect.x, r = t.rect.y, l = c + n, u = r + i;
  let a = 0, d = 0;
  switch (s.type) {
    case "top":
      switch (s.align) {
        case "start":
          a = r - o - e.rect.height, d = c;
          break;
        case "center":
          a = r - o - e.rect.height, d = c - e.rect.width / 2 + n / 2;
          break;
        case "end":
          a = r - o - e.rect.height, d = c + n - e.rect.width;
          break;
      }
      break;
    case "left":
      switch (s.align) {
        case "start":
          a = r, d = c - o - e.rect.width;
          break;
        case "center":
          a = r - e.rect.height / 2 + i / 2, d = c - o - e.rect.width;
          break;
        case "end":
          a = r + i - e.rect.height, d = c - o - e.rect.width;
          break;
      }
      break;
    case "right":
      switch (s.align) {
        case "start":
          ...
          break;
        case "center":
          a = r - e.rect.height / 2 + i / 2, d = l + o;
          break;
        case "end":
          a = r + i - e.rect.height, d = l + o;
          break;
      }
      break;
    case "bottom":
      switch (s.align) {
        case "start":
          ...
          break;
        case "center":
          a = u + o, d = c - e.rect.width / 2 + n / 2;
          break;
        case "end":
          a = u + o, d = c + n - e.rect.width;
          break;
      }
      break;
  }
...
function D(t) {
  const e = t.getBoundingClientRect(), s = e.y, o = window.innerHeight;
  let n = 0, i = 0, c = false;
  return s < 0 ? (n = 1 * (m(e.height, -s) / 100), n >= 1 && (n = 1)) : s > o - e.height ? (i = 1 * (m(e.height, s - (o - e.height)) / 100), i >= 1 && (i = 1)) : c = true, {
    rect: e,
    firstTopLastBottomHideFactor: n,
    firstBottomLastTopHideFactor: i,
    isFullVisible: c
  };
}
function j(t, e) {
...
function _(t) {
  const { targetElement: e, renderContainerClassName: s, onSuccess: o } = t, i = document.createElement("div");
  i.style.width = "1px", i.style.height = "1px", i.style.overflow = "hidden", i.style.position = "fixed", i.style.top = "-2400px", i.style.left = "-2400px", i.style.opacity = "0", i.style.visibility = "hidden", document.body.appendChild(i);
  const c = document.createElement("div");
  i.appendChild(c);
  const r = new MutationObserver(() => {
    if (!(u instanceof HTMLElement)) return;
    const a = u.getBoundingClientRect();
    typeof o == "function" && o({ width: a.width, height: a.height }), r.disconnect(), c.remove(), i.remove();
  });
  r.observe(c, { subtree: true, childList: true });
  const l = s.split(" ");
  for (const a of l)
    a.trim() !== "" && c.classList.add(a);
  const u = e.cloneNode(true);
  c.appendChild(u);
}
function F(t, e) {
  const s = t.getTime();
  ...
  if (o < s)
    return;
  const { isMoreDetail: n = false } = e ?? {}, i = o - s, c = Math.floor(i / 1e3), r = Math.floor(i / (1e3 * 60)), l = Math.floor(i / (1e3 * 60 * 60)), u = i % (1e3 * 60 * 60), a = Math.floor(i / (1e3 * 60 * 60 * 24)), d = i % (1e3 * 60 * 60 * 24), h = Math.floor(i / (1e3 * 60 * 60 * 24 * 365)), f = i % (1e3 * 60 * 60 * 24 * 365);
  let p = "", g = "";
  return r <= 0 ? p = `${c}초 전` : l <= 0 ? p = `${r}분 전` : a <= 0 ? (n === true && (g = `${Math.floor(u / (1e3 * 60))}분 `), p = `${l}시간 ${g}전`) : h <= 0 ? (n === true && (g = `${Math.floor(d / (1e3 * 60 * 60))}시간 `), p = `${a}일 ${g}전`) : (n === true && (g = `${Math.floor(f / (1e3 * 60 * 60 * 24))}개월`), p = `${h}년 ${g}전`), {
    diffTime: i,
    agoSecond: c,
    agoMinute: r,
    agoHour: l,
...
function I(t) {
  const { target: e, options: s, callback: o } = t, n = new MutationObserver(o);
  return n.observe(e, s), n;
}
function z(t, e) {
  const s = t.replace("v", "").split(".").map((n) => Number(n)), o = e.replace("v", "").split(".").map((n) => Number(n));
  return s[0] > o[0] ? "left-higher" : s[0] < o[0] ? "right-higher" : s[1] > o[1] ? "left-higher" : s[1] < o[1] ? "right-higher" : s[2] > o[2] ? "left-higher" : s[2] < o[2] ? "right-higher" : "same";
}
...
window.addEventListener("load", () => {
  const commentListDiv = document.querySelector(`#tt-body-page [data-tistory-react-app="Namecard"]`);
...
function commentLinkCopyButtonClick(element) {
  const dataLink = element.getAttribute("data-link");
  copy(`${location.host}${dataLink}`);
  alert("링크가 복사되었습니다.");
}
window.commentLinkCopyButtonClick = commentLinkCopyButtonClick;
...
function isAdmin() {
  ...
  if (window.tiara !== void 0) {
    if (((_a = window.tiara.customProps) == null ? void 0 : _a.role) === "owner") {
      return true;
    }
  }
  return false;
...
function latestVersionCheck(modalId) {
  if (!isAdmin()) {
    return;
  }
  if (isIframe()) {
  ...
  axios.get(`https://cdn.jsdelivr.net/gh/wisdomstar94/tistory-discord-ui-skin/package.json?t=${Date.now()}`).then((res) => {
    ...
    const versionCompareResult = z(version, onlyLatestVersion);
    ...
    if (versionCompareResult === "right-higher") {
      if (getLatestVversionHideTargetTime() <= Date.now()) {
        const modal2 = getModal(modalId);
        modal2 == null ? void 0 : modal2.classList.remove("modal-close");
        modal2 == null ? void 0 : modal2.classList.add("modal-open");
      }
    }
  });
}
window.latestVersionCheck = latestVersionCheck;
function negativeButtonClick(modalId) {
  localStorage.setItem("latest-version-hide-target-time", (Date.now() + 1e3 * 60 * 60 * 24 * 30).toString());
}
window.negativeButtonClick = negativeButtonClick;
function positiveButtonClick(modalId) {
  window.open(`https://github.com/wisdomstar94/tistory-discord-ui-skin/releases/latest`, "_blank");
  const modal = getModal(modalId);
  modal == null ? void 0 : modal.classList.remove("modal-open");
  modal == null ? void 0 : modal.classList.add("modal-close");
}
window.positiveButtonClick = positiveButtonClick;
...
function goToSkinLatestPage(modalId) {
  const modal = getModal(modalId);
  window.open("https://github.com/wisdomstar94/tistory-discord-ui-skin/releases/latest", "_blank");
  modal == null ? void 0 : modal.classList.remove("modal-open");
  modal == null ? void 0 : modal.classList.add("modal-close");
}
window.goToSkinLatestPage = goToSkinLatestPage;
window.addEventListener("load", () => {
  initPostsIndex();
});
...
window.postsIndexToggleButtonClick = postsIndexToggleButtonClick;
function initPostsIndex() {
  if (document.body.id !== "tt-body-page") {
    return;
  }
  const articleDiv = j(
    document.querySelector(`#article-description .contents_style`),
  ...
  headTags.forEach((headTag, index) => {
    ...
    const liTemplateList = j(
      document.querySelector('[data-title="posts-index-li-template-list"]'),
      `[data-title="posts-index-li-template-list"] 요소가 없습니다.`
    );
    const liTemplate = liTemplateList.querySelector(`[data-title='${headTag.nodeName.toLowerCase()}-li']`);
    if (liTemplate === null) {
    ...
    const onlyText = j(li.querySelector('[data-title="only-text"]'), `[data-title="only-text"] 요소가 없습니다.`);
...
window.checkDefaultDisplay = checkDefaultDisplay;
window.addEventListener("load", () => {
  const sideSmallBarScrollArea = document.querySelector(".side-big-bar-scroll-area");
  if (sideSmallBarScrollArea === null) return;
  sideSmallBarScrollArea.addEventListener("scroll", () => {
    const li = document.querySelector(".side-blog-profile-item");
    if (li === null) return;
    const info = D(li);
    const blogTitleBarFloatContainer = document.querySelector(".blog-title-bar-float-container");
    if (blogTitleBarFloatContainer === null) return;
    const movableBlogTitleBar = document.querySelector(".movable-blog-title-bar");
    if (movableBlogTitleBar === null) return;
    const movableProfileAreaOverlayBg = document.querySelector(".movable-profile-area-overlay-bg");
    if (movableProfileAreaOverlayBg === null) return;
    const bg = blogTitleBarFloatContainer.querySelector(".background");
    if (bg === null) return;
    const backgroundBottomBorder = blogTitleBarFloatContainer.querySelector(".background-bottom-border");
    if (backgroundBottomBorder === null) return;
    const profileImage = document.querySelector(".side-big-bar-scroll-area .profile-image");
  ...
  j(document.querySelector(`#aside-side-bar`), "sidebar 요소가 없습니다.").style.height = "100vh";
});
function checkActiveCategory() {
  const ul = j(document.querySelector("ul.tt_category"), "ul.tt_category 요소가 없습니다.");
  const { isCategoryPath, categoryType, categoryName, subCategoryName } = getCategoryPathnameInfo();
  if (!isCategoryPath) return;
  if (categoryType === "all") {
    const linkTit = j(ul.querySelector("a.link_tit"), "a.link_tit 요소가 없습니다.");
    linkTit.classList.add("active");
  } else if (categoryType === "category") {
    const linkItems = j(ul.querySelectorAll("a.link_item"), "a.link_item 요소가 없습니다.");
    linkItems.forEach((item) => {
      var _a, _b;
      const copyItem = item.cloneNode(true);
      (_a = copyItem.querySelector(".c_cnt")) == null ? void 0 : _a.remove();
      const renderedCategoryName = ((_b = copyItem.textContent) == null ? void 0 : _b.trim()) ?? "";
      if (renderedCategoryName.length >= 27) {
        if (categoryName == null ? void 0 : categoryName.startsWith(renderedCategoryName.slice(0, renderedCategoryName.length - 2))) {
    ...
    linkItems.forEach((item) => {
      ...
      const copyItem = item.cloneNode(true);
      (_a = copyItem.querySelector(".c_cnt")) == null ? void 0 : _a.remove();
      const renderedParentCategoryName = ((_b = copyItem.textContent) == null ? void 0 : _b.trim()) ?? "";
      const subLinkItems = j((_c = item.parentElement) == null ? void 0 : _c.querySelectorAll("a.link_sub_item"), "a.link_sub_item 요소가 없습니다.");
      subLinkItems.forEach((item2) => {
        var _a2, _b2;
        const copyItem2 = item2.cloneNode(true);
        (_a2 = copyItem2.querySelector(".c_cnt")) == null ? void 0 : _a2.remove();
        const renderedChildCategoryName = ((_b2 = copyItem2.textContent) == null ? void 0 : _b2.trim()) ?? "";
        if (renderedParentCategoryName.length >= 27) {
          if (categoryName == null ? void 0 : categoryName.startsWith(renderedParentCategoryName.slice(0, renderedParentCategoryName.length - 2))) {
            if (renderedChildCategoryName.length >= 27) {
              if (subCategoryName == null ? void 0 : subCategoryName.startsWith(renderedChildCategoryName.slice(0, renderedChildCategoryName.length - 2))) {
...
window.mobileSideBarBackgroundClick = mobileSideBarBackgroundClick;
function sideBarCloseButtonClick() {
  document.body.classList.add("side-bar-close");
  document.body.classList.remove("side-bar-open");
}
window.sideBarCloseButtonClick = sideBarCloseButtonClick;
function checkTopBarTitle() {
  j(document.querySelector(".top-bar-title-area"), ".top-bar-title-area 요소가 없습니다.");
}
window.checkTopBarTitle = checkTopBarTitle;
...
window.sideBarToggleButtonClick = sideBarToggleButtonClick;
function modalBgClick(id) {
  closeModal(id);
}
function modalCancelButtonClick(id) {
  closeModal(id);
}
function closeModal(id) {
  const modalContainer = j(document.querySelector(`#${id}`), "해당 modal 이 없습니다.");
  modalContainer.classList.remove("modal-open");
  modalContainer.classList.add("modal-close");
}
window.modalBgClick = modalBgClick;
window.modalCancelButtonClick = modalCancelButtonClick;
function checkPageTitle() {
  ...
  if (document.body.id === "tt-body-category") {
    ...
    if (isCategoryPath) {
      ...
      const span = document.querySelector('[data-title="category-title"] > .only-title-text');
      if (span !== null) {
...
window.checkPageTitle = checkPageTitle;
window.addEventListener("load", () => {
  const articleList = document.querySelector('[data-title="article-list"]');
...
function initialAnotherCategoryPosts() {
  const anotherCategory = document.querySelector(`#tt-body-page .another_category`);
  if (anotherCategory === null) return;
  const trs = anotherCategory.querySelectorAll("table tbody tr");
  if (trs.length === 0) return;
  trs.forEach((tr) => {
    tr.addEventListener("click", () => {
      const a = j(tr.querySelector("a[href]"), "a[href] 요소가 없습니다.");
      const href = j(a.getAttribute("href"), "href 가 없습니다.");
      location.href = href;
    });
  });
}
function initialCodeBlock() {
  const codes = document.querySelectorAll("#article-description pre code");
  codes.forEach((code) => {
    var _a;
...
function settingHoverElement() {
  const elements = document.querySelectorAll(".small-symbol-button-popover-container");
...
function handleSideItemAButtonMouseOver(target) {
  var _a;
  settingHoverElement();
  const popoverMessageContainer = (_a = target.parentElement) == null ? void 0 : _a.querySelector(".popover-message-container");
  if (popoverMessageContainer === null || popoverMessageContainer === void 0) {
...
window.handleSideItemAButtonMouseOver = handleSideItemAButtonMouseOver;
function goToTop() {
  window.scrollTo({
    top: 0,
    behavior: "smooth"
